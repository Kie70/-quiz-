        ...
        {logoutConfirmOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="logout-confirm-title">
            <div className="rounded-xl border border-zinc-700 bg-zinc-900 shadow-xl max-w-sm w-full p-6">
              ...
              <div className="flex gap-3 justify-end">
                <button type="button" onClick={cancelLogout} className="px-4 py-2 rounded-lg border border-zinc-600 text-zinc-300 text-sm hover:bg-zinc-800 transition-colors">取消</button>
                <button type="button" onClick={confirmLogout} className="px-4 py-2 rounded-lg bg-white text-zinc-950 font-medium text-sm hover:bg-zinc-200 transition-colors">退出</button>
              </div>
            </div>
          </div>
        )}
        <header className="sticky top-0 z-20 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-sm">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between max-w-[650px]">
            <nav className="flex items-center space-x-8">
              ...
              {isAdmin && <NavLink to="admin" label="管理后台" />}
            </nav>
            <button
              type="button"
